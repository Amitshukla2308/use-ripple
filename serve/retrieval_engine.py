...
def initialize(
    ...
    if G is None:
        ...
        if cochange_path.exists():
            ...
            # T-040: detect whether index has accumulated decay_weight field
            global cochange_has_decay
            sample_edges = next(iter(cochange_index.get("edges", {}).values()), [])
            cochange_has_decay = bool(sample_edges and "decay_weight" in sample_edges[0])
            if cochange_has_decay:
                print("  co-change index: decay_weight field present (T-040 dual-weight)")
        else:
...
def predict_missing_changes(changed_modules: list, min_weight: int = 3,
    ...
    for mod in changed_set:
        ...
        for nb in neighbors:
            ...
            w = _cc_weight(nb)  # T-040: accumulated decay when available, flat count otherwise
