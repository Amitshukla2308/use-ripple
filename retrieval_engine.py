...
# ════════════════════════════════════════════════════════════════════════════
# ROUTER + AGENT TOOL SCHEMAS
# ════════════════════════════════════════════════════════════════════════════

ROUTER_TOOL = [{"type": "function", "function": {
    "name": "route_and_expand",
    "description": (
        "Select the best expert persona AND expand the query for retrieval. "
        "Called once per user question — do not produce any text, only call this tool."
    ),
    "parameters": {"type": "object", "properties": {
        "persona": {
            "type": "string",
            "enum": ["domain_expert", "new_engineer_guide", "reliability_engineer",
                     "security_auditor", "bar_raiser", "observability_engineer",
                     "architect", "senior_dev"],
            "description": (
                "domain_expert: architecture, data flow, how things work. "
                "new_engineer_guide: onboarding, where to start, orientation. "
                "reliability_engineer: blast radius, failure modes, cascading failures. "
                "security_auditor: credentials, auth, PCI-DSS, token lifecycle. "
                "bar_raiser: code quality, design review, principle adherence. "
                "observability_engineer: debugging, incidents, root cause analysis. "
                "architect: designing WHERE new code should live — module boundaries, extension points, structural decisions. "
                "senior_dev: concrete code changes — exact files, functions, ripple effects, implementation patterns."
            )
        },
        "intent":   {"type": "string", "description": "One sentence: exactly what the engineer needs."},
        "focus":    {"type": "string", "description": "Key concept, function, or service at the centre of this question."},
        "query_variants": {
            "type": "array", "items": {"type": "string"},
            "description": "3 alternative retrieval queries targeting language-specific naming conventions.",
        },
        "relevant_services": {
            "type": "object",
            "description": "Service → relevance weight (0.0–1.0), weights sum ~1.0.",
            "additionalProperties": {"type": "number"},
        },
    }, "required": ["persona", "intent", "query_variants", "relevant_services"]}
}}]

# ── Keyword-based fast router (no LLM) ──────────────────────────────────────
# Single persona — routing only affects service_weights for search focus.


_INSTRUCTION_TAIL_RE = re.compile(
    r"\s+and\s+(write|explain|describe|note|summarize|summarise|"
    r"give|tell|show|list|create|make|generate|provide|produce|draft)\b.*$",
    re.IGNORECASE,
)
_INSTRUCTION_PREFIX_RE = re.compile(
    r"^(find\s+out|tell\s+me(\s+about)?|show\s+me|walk\s+me\s+through|"
    r"how\s+does|how\s+do|what\s+is|what\s+are|what\s+does|"
    r"which\s+function|which\s+class|which\s+service|which\s+module|"
    r"explain|describe|list|find|trace|write\s+a|give\s+me)\s+",
    re.IGNORECASE,
)


_TRAILING_FILLER_RE = re.compile(
    r"\s+(works?(\s+end\s+to\s+end)?|end\s+to\s+end|e2e|step\s+by\s+step|in\s+detail|in\s+depth)\s*$",
    re.IGNORECASE,
)
...
def _extract_query_subject(query: str) -> str:
    """
    Strip instruction framing and return only the domain-concept part.

    'Find out all payment flows and write a short note on each'
      → 'all payment flows'
    'Explain UPI collect flow end to end'
      → 'UPI collect flow end to end'
    """
    q = _INSTRUCTION_TAIL_RE.sub("", query).strip()
    q = _INSTRUCTION_PREFIX_RE.sub("", q).strip()
    q = _TRAILING_FILLER_RE.sub("", q).strip()
    # Strip leading quantifiers that add no retrieval signal
    q = re.sub(r"^(all|every|the|a|an)\s+", "", q, flags=re.IGNORECASE).strip()
    return q


def _query_variants_heuristic(query: str) -> list:
    """Generate Haskell/Rust naming-convention variants without an LLM call."""
    subject = _extract_query_subject(query)

    stopwords = {"what", "how", "does", "which", "where", "when", "this", "that",
                 "with", "from", "into", "about", "explain", "show", "tell", "give",
                 "list", "find", "trace", "walk", "through", "the", "a", "an", "is",
                 "are", "do", "i", "me", "we", "our", "each", "every", "them",
                 "note", "short", "brief", "write", "ones", "those", "these",
                 "work", "works", "working", "change", "changes", "radius",
                 "end", "step", "steps", "path", "paths", "handler", "handlers",
                 # comparison / structural connectors — never useful as symbol name components
                 "between", "written", "difference", "differences", "versus", "compare",
                 "comparison", "and", "in", "of", "by", "for", "per", "at", "on",
                 "also", "same", "different", "similarities", "similar", "across",
                 "between", "used", "use", "uses", "using", "based", "related"}
    words = [w for w in subject.lower().split()
             if (len(w) > 3 or w in _KW_ALLOWLIST)
             and w not in stopwords
             and "." not in w    # skip file paths like Routes.hs
             and "-" not in w]   # skip service names like euler-api-txns
    if not words:
        return []

    # If a known gateway is in the query, generate gateway-specific variants
    gw = next((w for w in words if w in _KNOWN_GATEWAYS), None)
    if gw:
        gw_cap = gw.capitalize()
        return [f"{gw_cap}Routes", f"{gw_cap}Flow", f"{gw_cap}Gateway"]

    core_words = words[:3]   # max 3 — 4 produces nonsense for long subjects
    camel   = core_words[0] + "".join(w.capitalize() for w in core_words[1:])
    pascal  = "".join(w.capitalize() for w in core_words)
    haskell = ".".join(w.capitalize() for w in core_words[:3])
    return list(dict.fromkeys([camel, pascal, haskell]))[:3]  # dedup while preserving order


_ARCHITECTURAL_SIGNALS = {
    "map", "entire", "complete", "full", "end-to-end", "e2e", "end to end",
    "all services", "across services", "across all", "every service",
    "architecture", "overview", "landscape", "ecosystem",
    # comparison queries spanning multiple services are inherently architectural
    "difference between", "compare", "comparison", "versus", "vs", "differ",
}
_TARGETED_SIGNALS = {
    "what does", "what is", "show me", "where is", "define", "definition",
    "explain", "signature", "type of", "which file", "which module",
    "which function", "which class", "which service",
}


def _estimate_complexity(query: str, service_weights: dict) -> tuple[str, int]:
    """
    Estimate investigation complexity from query text and service spread.
    Returns (depth_label, max_tool_calls).
      targeted     →  5  (single-function lookup, definition question)
      flow         → 10  (how does X work, trace X through a service)
      architectural → 18  (map entire X, end-to-end, cross-service)
    """
    q = query.lower()
    num_services = len(service_weights)

    # Architectural: explicit "map / entire / full flow" language OR spans 4+ services
    if num_services >= 4 or any(sig in q for sig in _ARCHITECTURAL_SIGNALS):
        return "architectural", 18

    # Targeted: lookup-style phrasing AND at most 1 service
    if num_services <= 1 and any(q.startswith(sig) or f" {sig} " in q for sig in _TARGETED_SIGNALS):
        return "targeted", 5
...
# Service name aliases — patterns the user types vs canonical names in the graph
_SVC_ALIASES: dict[str, str] = {
    "euler-api-txns":          "euler-api-txns",
    "api-txns":                "euler-api-txns",
    "api txns":                "euler-api-txns",
    "euler-api-gateway":       "euler-api-gateway",
    "api-gateway":             "euler-api-gateway",
    "api gateway":             "euler-api-gateway",
    "ucs":                     "UCS",
    "euler-api-order":         "euler-api-order",
    "api-order":               "euler-api-order",
    "api order":               "euler-api-order",
    "euler-api-pre-txn":       "euler-api-pre-txn",
    "api-pre-txn":             "euler-api-pre-txn",
    "pre-txn":                 "euler-api-pre-txn",
    "euler-api-customer":      "euler-api-customer",
    "api-customer":            "euler-api-customer",
    "euler-db":                "euler-db",
    "graphh":                  "graphh",
    "euler-drainer":           "euler-drainer",
    "drainer":                 "euler-drainer",
    "basilisk":                "basilisk-v3",
    "basilisk-v3":             "basilisk-v3",
    "haskell-sequelize":       "haskell-sequelize",
    "sequelize":               "haskell-sequelize",
    "token-issuer":            "token_issuer_portal_backend",
    "token issuer":            "token_issuer_portal_backend",
}


def _extract_mentioned_services(query: str) -> dict[str, float]:
    """
    Detect service names / abbreviations explicitly typed by the user.
    Returns {canonical_svc: weight} with equal weights summing to 1.0.
    Empty dict if no service mentions found.
    """
    q = query.lower()
    found: set[str] = set()
    ...
    if not found:
        return {}
    w = round(1.0 / len(found), 2)
    return {svc: w for svc in sorted(found)}


def fast_route(query: str, kw_hits: list) -> tuple:
    """
    Route query — always returns juspay_code persona.
    Computes service_weights from explicit service mentions first, then keyword hits.
    Returns (persona, tool_args, query_variants, service_weights, max_tool_calls).
    """
    # 1. Explicit service names typed by the user take highest priority
    service_weights: dict = _extract_mentioned_services(query)

    # 2. Fall back to keyword-hit distribution when no explicit names found
    if not service_weights:
        svc_counts: dict = {}
        for node in kw_hits:
            svc = node.get("service", "")
            if svc and not _is_test_path(node.get("file", "")):
                svc_counts[svc] = svc_counts.get(svc, 0) + 1
        if svc_counts:
            total = sum(svc_counts.values())
            service_weights = {s: round(c / total, 2) for s, c in
                               sorted(svc_counts.items(), key=lambda x: -x[1])[:4]}

    query_variants = _query_variants_heuristic(query)
    tool_args = {"intent": query, "focus": ""}
    depth_label, max_tool_calls = _estimate_complexity(query, service_weights)
    return "juspay_code", tool_args, query_variants, service_weights, max_tool_calls, depth_label
...
def _encode_queries_batch(queries: list) -> list:
    """
    Encode multiple queries in ONE HTTP call to the embed server.
    Returns a list of vectors in the same order as queries.
    Falls back to sequential local encoding when embed server is unavailable.
    """
    if not queries:
        return []
    ...
    if EMBED_SERVER_URL:
        try:
            ...
            return resp["embeddings"]   # list of vectors, same order as queries
        except Exception as e:
    ...
    return [[] for _ in queries]   # keyword-only mode


def doc_search_for_query(query: str, top_k: int = 20) -> list:
    """Encode + doc vector search in one call — safe to pass directly to asyncio.to_thread."""
    qvec = _encode_query(query)
    return doc_vector_search(qvec, top_k) if qvec else []
